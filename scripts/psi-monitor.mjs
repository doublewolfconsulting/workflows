...
async function diagnose(mobileData, desktopData, mobile, desktop, siteAudit) {
...
function buildBody(mobile, desktop, siteAudit, diagnosis, fixPr) {
  const lines = [];
...
async function main() {
  ...
  const allPass = psiPass && auditPass;
  ...
  if (ANTHROPIC_API_KEY) {
    const result = await diagnose(finalMobileData, finalDesktopData, mobile, desktop, siteAudit);
    if (result) {
  ...
  if (openIssue) {
    ...
    const issue = await gh('POST', '/repos/' + OWNER + '/' + REPO + '/issues', {
      title: title,
      body: buildBody(mobile, desktop, siteAudit, diagnosis, null),
      labels: [LABEL],
    });
  ...
  const body = buildBody(mobile, desktop, siteAudit, diagnosis, fixPr);
  if (openIssue) {
